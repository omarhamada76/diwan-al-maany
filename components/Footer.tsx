...
export default function Footer({ locale }: FooterProps) {
  ...
  return (
    <footer className="bg-[#111111] border-t border-[#D4AF37]/10 text-[#9E9E9E] relative overflow-hidden z-10">
      
      {/* Visual Accent Light */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-[150px] bg-gradient-to-t from-[#D4AF37]/5 to-transparent rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8 mb-12">
          ...
          <div className="md:col-span-4 flex flex-col gap-6">
            <Link href="/" className="flex items-center gap-3">
              <div className="w-9 h-9 border border-[#D4AF37] rounded-full flex items-center justify-center bg-[#090909]">
                <Shield className="w-4.5 h-4.5 text-[#D4AF37]" />
              </div>
              <div className="flex flex-col">
                <span className="text-base font-bold tracking-widest text-[#F5F5F5]">
                  {locale === 'ar' ? 'ديوان المعاني' : 'DIWAN AL MAANY'}
                </span>
                <span className="text-[8px] tracking-[0.2em] text-[#D4AF37] uppercase">
                  {t('tagline')}
                </span>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-4 mt-2">
              {socialLinks.map((s) => (
                <a
                  key={s.name}
                  href={s.href}
                  className="text-xs text-[#9E9E9E] hover:text-[#D4AF37] flex items-center gap-1 transition-colors duration-300"
                >
                  {s.name}
          ...
          <div className="md:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-8">
            <div className="flex flex-col gap-3">
              <span className="text-xs font-semibold text-[#F5F5F5] uppercase tracking-wider border-b border-[#D4AF37]/20 pb-2">
                {tContact('addressTitle')}
              </span>
              <div className="flex gap-2 text-xs">
                <MapPin className="w-4 h-4 text-[#D4AF37] shrink-0" />
                <p className="leading-relaxed text-[#9E9E9E]">
                  {tContact('address')}
                </p>
              </div>
            </div>
          ...
          <div className="md:col-span-3 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 text-xs">
              <a
                ...
                className="flex items-center gap-2 text-[#9E9E9E] hover:text-[#D4AF37] transition-colors"
              >
        ...
        {/* Lower Banner */}
        <div className="border-t border-[#D4AF37]/10 pt-8 mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-[10px]">
